...
void Test3() {
...

  {
    ...
    s21::Vector<Obj> v_medium(MEDIUM_SIZE);

    v_medium[MEDIUM_SIZE / 2].id = ID;

    s21::Vector<Obj> v_large(LARGE_SIZE);
...

  {
    ...
    s21::Vector<Obj> v_small(MEDIUM_SIZE / 2);
...
void Test6() {
...

  {
    ...
    s21::Vector<Obj>::iterator pos = v.Insert(v.cbegin() + 1, obj);
...

  {
    ...
    s21::Vector<Obj>::iterator pos = v.Insert(v.cbegin() + 1, Obj{1});
  ...
  {
    s21::Vector<TestObj> v{SIZE};

    v.Insert(v.cbegin() + 2, v[0]);

    assert(std::all_of(v.begin(), v.end(),
                       [](const TestObj &obj) { return obj.IsAlive(); }));
  }
...
void Dump() {
  ...
  cerr << "Def ctors: "sv << C::def_ctor << ", Copy ctors: "sv << C::copy_ctor
       << ", Move ctors: "sv << C::move_ctor << ", Copy assignments: "sv
       << C::copy_assign << ", Move assignments: "sv << C::move_assign
